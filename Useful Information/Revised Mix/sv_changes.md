...
# 2026-08-12d — Output gain + trigger-polarity audit (FPGA bring-up)

**Trigger:** first FPGA deployment: output nearly silent at normal monitor
volume, and a wave-launch sound that differs from MAME ("short sonar-type
sound" vs "2 player shots").

## Volume: OUTPUT_GAIN_LOG2 added (default 2, +12 dB)

Diagnosis: not a bug — an unhandled consequence of board-authentic mixing.
The summing weights are true to the board's resistors, which puts the
LOUDEST voice (a clamped explosion) at only −8.7 dBFS and the invader
march ~35 dB below that, near −43 dBFS. On a real cabinet the power amp
and volume pot supplied the missing absolute gain; a line-level DAC feed
does not. Fix: a master gain applied AFTER the authentic mix, before the
existing output clamp — relative levels untouched. Settings documented in
the code: 0 = never clips (very quiet), 1 = explosions still unclipped,
2 = default +12 dB (explosion bodies clip ~30% over — they are already
diode-clamped noise thumps, so the extra crunch reads as cabinet
loudness), 3 = "operator cranked it". Regression tb retargeted
(invader peak band now ~900) and passing.

## Trigger audit vs nl_astrob.cpp: NO DISCREPANCY

Full chain checked — latch bit → input buffer (U26/U29 = 7407
non-inverting; U30/U31 = 7406 inverting) → circuit activation sense →
SV implementation:

| Latch bit | Buffer | Circuit sense | SV | Match |
|---|---|---|---|---|
| 3E.0–3 INV1–4 | 7406 inv | runs when bit LOW | level `~bit` | ✓ |
| 3E.4 ASTRO | 7406 inv | on when bit LOW | level `~bit` | ✓ |
| 3E.5 MUTE | raw | active HIGH | `bit` | ✓ |
| 3E.6 REFILL | 7407 + 7406 shunt | sounds when bit LOW | level `~bit` | ✓ |
| 3E.7 WARP | raw + 7406 | active HIGH (capture-anchored) | `bit` | ✓ |
| 3F.0–3 LZR1/LZR2/SEXPL/LEXPL | 7407 | one-shot on bit FALLING | falling edge | ✓ |
| 3F.4 ATTACK_RATE | 7407 | staircase runs while bit LOW | level `~bit` | ✓ |
| 3F.5 RATE_RESET | 7406 inv | CD4029 reset (active-high pin) when bit LOW | level `~bit` | ✓ |
| 3F.6 BONUS | 7407 | one-shot on bit FALLING | falling edge | ✓ |
| 3F.7 SONAR | 7407 | one-shot on bit FALLING | falling edge | ✓ |

Address decode ($3E even → latch_3e, $3F odd → latch_3f) also correct,
and confirmed empirically by the marches sounding right on hardware.

## The wave-launch difference IS MAME's bug, not ours

nl_astrob.cpp's own header: "SONAR and BONUS sounds fail to trigger …
the triggering via CD4011 never actually fires the one-shot." Both
implementations receive identical latch writes, so the only voices that
can differ are the two broken in exactly one of them. At sector launch
the game evidently pulses SONAR together with both lasers: MAME plays
only the lasers ("2 player shots"); this core plays all three — and the
sonar's mixer weight (350) beats LASER_1+LASER_2 (164+77) by **15.7 dB
measured**, masking them. Verification render:
`astrob_launch_event.wav` pulses bits 0, 1 and 7 low in one write — if
it matches what the FPGA does at launch, the explanation is confirmed.
A REAL board should also ping there (the MAME failure is a netlist
simulation artifact, not a hardware behaviour); ground truth would be a
real-cabinet gameplay recording, not MAME.

## Deliverables

`astrob_audio.sv` (rev 2026-08-12d), `tb_astrob.sv` (retargeted),
`sim_launch.cpp`, `astrob_launch_event.wav`.


---

# 2026-08-12e — MM5837 LFSR lockup guard

**Trigger:** the full-showcase render's explosions and asteroids played as
DC pulses — "speaker connected to DC then disconnected," exactly as
reported. Measurement confirmed: every loud sample negative, −13k to −19k
DC, no content above 20 Hz, while the tone voices in the same file were
healthy.

**Root cause:** a XOR-feedback LFSR has an absorbing all-zero state. The
showcase harness reset the DUT with bare `eval()` calls instead of clocked
ticks, so the async-reset branch never fired, every register zero-
initialised, and the LFSR entered its dead state — `noise_bit` stuck low
turned both noise VCAs into −envelope DC. Probe confirmed: `lfsr=00000`
under the faulty harness, running under a clocked reset. **The RTL on
FPGA was never affected** (the wrapper asserts reset — which is why
explosions sounded correct in the first hardware test), and only the two
noise voices could die because the LFSR is the design's sole absorbing
state; everything else recovers from zero-init.

**Fixes, defense in depth:**
1. RTL hardened: the feedback term gains `| (lfsr == 0)` — self-starting
   from any state, including a cold power-up where reset never pulses.
   Verified by re-running the exact faulty un-reset sequence: the LFSR now
   escapes zero and runs.
2. Harness fixed to clocked reset.
3. Showcase re-rendered and re-verified: noise segments now sign-balanced
   (~50/50), DC residual <3% of peak, spectral peaks in the 48–81 Hz
   filter regions. Regression tb 4/4.

**Deliverables:** `astrob_audio.sv` (rev 2026-08-12e),
`astrob_full_showcase.wav` (rebuilt), `sim_show.cpp` (fixed).


---

# 2026-08-12f — ATTACK_RATE polarity fixed; MIXE compression reverted

**Trigger:** core-side review of 12d/12e plus first extended MiSTer
session ("all sounds present but the mix is off — cannot hear the
explosions over the invader noise").

## ATTACK_RATE polarity: the core-side review was right, my 12d audit was wrong

`NET_C(I_ATTACK_RATE, R148.1, U21.5)` puts the RAW bit (U26 7407,
non-inverting) on a CD4011 NAND astable gate, and a NAND astable runs
when its gate is HIGH — LOW pins the output and freezes it. So **bit
HIGH = staircase climbs**, and `attack_run = ~latch_3f[4]` was backwards.
The 12d audit table row claiming "runs while bit LOW" through a
non-inverting buffer was self-contradictory, exactly as the review said;
my own code comment ("R148 high") and the showcase measurement (staircase
climbing while bit 4 was held low) were both symptoms in plain sight.
Fixed to `attack_run = latch_3f[4]`, comments corrected, and one noted
consequence: latch_3f resets to 8'hFF, so the staircase free-runs from
power-up until the game's first $3F write — equivalent to a real board's
arbitrary latch power-up state, resolved within milliseconds by game init.

## The MIXE_* cube-root compression: not adopted, and here is the ear-test

The core-side change replaced the board-resistor weights (MIXW_*) with a
3:1 dB compression (46.6 → 15.5 dB spread). The underlying caveat is
legitimate — MIXW assumes every voice reaches the summing node at equal
Vpp, and the board mixes full-rail 555/CMOS voices with attenuated
op-amp/coupled ones — but the correction magnitude is not: equal-Vpp
error is bounded at a few dB per voice, while the compression boosts each
invader march voice by **+31 dB relative** (weight 77 → 2743). Result on
hardware, as reported: four marching invaders sum to within ~2 dB of a
clamped explosion, and the march masks the explosions. That masking IS
the compression, not the board — the 1M-vs-4.7k summing resistors exist
precisely to make the march background. Reverted to MIXW. The right way
to settle the equal-Vpp question remains per-stage Vpp measurement (MAME
netlist node captures or a scoped board), applied as small per-voice
source factors — never a spread compression.

## Adopted from the review

The single-shift mixer (`>>> (14-1-2)` instead of `>>>13 <<<2`): identical
levels, two more LSBs of resolution for the quiet voices. Also added the
integration note it surfaced: **SegaG80.sv halves the astrob+speech sum**,
so OUTPUT_GAIN_LOG2 = 2 arrives at the core output as +6 dB, not +12. If
more absolute level is wanted, prefer core/OSD gain over raising
OUTPUT_GAIN_LOG2 to 3 — at 3 the asteroid bed also reaches the clamp,
which re-flattens the explosion-over-bed margin. A sonar-slice robustness
comment was added per the review's fragility note.

## Answer to the review's open question

The Aug-12 "DC thump / broken sound" was the **showcase render only** —
an unclocked-reset harness bug (12e) that zero-locked the LFSR in
simulation. Hardware always receives reset (as the review independently
confirmed from Arcade-SegaG80.sv:368) and was never affected.

## Levels at final settings (MIXW, makeup 1, gain 2, single shift)

At audio_out (halve for post-core): explosion 0 dBFS (clamped),
asteroids −3.2, launch combo −29, march (4 voices) −31.2, sonar −30.6,
laser1 −36.6, bonus −38.7, refill −42.6, laser2/single-invader −43.2.
Explosion over march: **31.2 dB** (was ~−2 dB under MIXE). Verified in
the re-rendered showcase, along with the corrected staircase gating
(104.0 Hz frozen at step 0 with bit 4 low, climbing with bit 4 high).
Regression tb 4/4.

**Remaining ear-check knob:** if the asteroid field still masks explosion
thumps in gameplay (they sit 3.2 dB apart at peak, in the same 60–100 Hz
band, transient vs continuous), the calibrated knob is ASTRO_GAIN_LOG2
2 → 1 (−6 dB on the asteroid voice). That value was netlist-nominal, not
capture-anchored — trimming it is legitimate calibration, unlike touching
the board-derived MIXW resistor weights.
