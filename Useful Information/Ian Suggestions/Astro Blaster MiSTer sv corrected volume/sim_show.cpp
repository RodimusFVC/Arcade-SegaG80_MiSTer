...
int main(int argc,char**argv){
    ...
    for(int v=0;v<4;v++){
        wr(1,HI_IDLE&~(1u<<5)); wr(1,HI_IDLE);      // RATE_RESET pulse -> step 0
        wr(0,LO_IDLE&~(1u<<v));                     // march on, staircase FROZEN
        run_s(1.5);
        wr(1,HI_IDLE&~(1u<<4));                     // ATTACK_RATE: climb
        run_s(1.5);
        idle(); run_s(0.4);
    }
    // all four together: same shape
    wr(1,HI_IDLE&~(1u<<5)); wr(1,HI_IDLE);
    wr(0,LO_IDLE&~0x0F); run_s(1.5);
    wr(1,HI_IDLE&~(1u<<4)); run_s(1.5); idle(); run_s(0.4);
