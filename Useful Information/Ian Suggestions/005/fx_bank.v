...
module fx_bank #(
    ...
    wire signed [17:0] heli_ch = vca_wide(noise2, heli_gain);

    assign ch_lexpl      = vca(bp_lexpl, env_lexpl);
    assign ch_sexpl      = vca(bp_sexpl, env_sexpl);
    assign ch_missile    = vca(bp_miss,  env_miss);
    assign ch_shoot      = vca(bp_shoot, env_shoot);
    assign ch_bomb       = vca(sq_bomb,  env_bomb);
    assign ch_whistle    = vca(sq_whis,  env_whis);
    assign ch_helicopter = vca(heli_ch,  env_heli);
