...
//
//  REVISION 2026-08-11 (this file): MIXER REWORKED — U7's eleven summing
...
module astrob_audio (
    ...
    //
    // ⚠ WARP-POLARITY OPEN QUESTION — now parameterised, still open.
    // The schematic-solved warp path (I_WARP/R164 shifting U16 pin 5)
    // moves V DOWN under warp -> pitch UP, which CONTRADICTS the
    // capture-derived 104.3->90.3 Hz drop this file encodes.
    //
    // Independent solve 2026-08-11 comes down on the schematic side:
    // U16 pin 5 sits at 12 * (1/4.7k)/((1/4.7k)+(1/10k)) = 8.163 V with
    // the 7406 off, and 6.186 V when R164 pulls it low — a 1.98 V DROP,
    // so V drops and pitch rises. Note also that the suspect package on
    // the capture board, U31, IS the warp inverter, which makes a warp-
    // specific fault the more economical explanation than a wrong
    // schematic. That is circumstantial, not proof, so the capture-derived
    // behaviour remains the DEFAULT and nothing audible changes here.
    //
    // To resolve: on a WORKING board, capture INV4 with warp on at a known
    // step. Pitch UP ~30% -> set WARP_SCHEMATIC = 1'b1 below. Pitch DOWN
    // -> leave it 0 and delete this note.
    //------------------------------------------------------------------------
    // 0 = capture-derived (pitch drops under warp)  <-- current default
    // 1 = schematic-derived (pitch rises under warp)
    //
    // Under branch 1 the warp tables are not used at all. The solved pin-5
    // drop of -1.977 V referred through the stage's non-inverting gain
    // (1 + 0.671) is a -3.304 V shift on V, i.e. 7.570 -> 4.266 at step 0,
    // which the 555 CV model turns into a half-period ratio of 0.7147
    // (pitch +39.9%). Applied as one Q12 constant to the NORMAL tables so
    // the A/B test is a single-bit change, not a re-derivation.
    localparam WARP_SCHEMATIC = 1'b0;
    localparam [12:0] WARP_RATIO_Q12 = 13'd2927;   // 0.7147 * 4096
    ...
    wire [29:0] inv1_wsch_p  = rom_i1n * WARP_RATIO_Q12;
    wire [17:0] inv1_period  =
        !warp_active     ? rom_i1n :
        WARP_SCHEMATIC   ? inv1_wsch_p[29:12] : rom_i1w;
    ...
    wire [29:0] inv3_wsch_p  = rom_i3n * WARP_RATIO_Q12;
    wire [16:0] inv3_per_ctr =
        !warp_active     ? rom_i3n :
        WARP_SCHEMATIC   ? inv3_wsch_p[28:12] : rom_i3w;
    ...
    wire signed [15:0] expl_out   = 16'sd0;
    wire signed [15:0] astro_out  = 16'sd0;
    wire signed [15:0] sonar_out  = 16'sd0;
    wire signed [15:0] bonus_out  = 16'sd0;
    wire signed [15:0] refill_out = 16'sd0;
    wire signed [15:0] laser1_out = 16'sd0;
    wire signed [15:0] laser2_out = 16'sd0;
    ...
    // MIX_Q14 = 14 would put a full-scale EXPLOSION at exactly +/-VOICE_FS.
    // MIX_MAKEUP_LOG2 is TEMPORARY headroom recovery: right now only the
    // -46.6 dB voices exist, so unity scaling would leave the output near
    // silent and the user would reasonably conclude the change broke it.
    //
    // >>> 7 puts the current four-invader peak at ~14400, comfortably loud
    // with 2x headroom left. DROP MIX_MAKEUP_LOG2 TO 0 the moment
    // EXPLOSIONS or ASTEROIDS is implemented, or they will clip instantly:
    // one full-scale explosion at makeup 7 wants 6000*16384>>7 = 768000.
    localparam MIX_Q14          = 14;
    localparam MIX_MAKEUP_LOG2  = 7;    // <-- SET TO 0 WHEN EXPL/ASTRO LAND
