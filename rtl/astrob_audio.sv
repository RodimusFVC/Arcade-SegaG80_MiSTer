...
//  NOT adopted: it boosts the invader march +31 dB relative and was
//  heard on hardware as the march masking explosions. The equal-Vpp
//  caveat behind it is real but bounded (a few dB per voice), to be
//  settled by per-stage Vpp measurement, not spread compression. See
//  sv_changes.md 2026-08-12f for the level table.
//  PRIOR REVISION 2026-08-12e: MM5837 LFSR lockup guard — the
...
//  not a real board; a hardware capture supersedes them (see the SONAR
//  block). BONUS ("the deedle") is netlist-nominal: 555 alternating
//  1501/1448 Hz at OSC4's 7.19 Hz, envelope = C34 recovering through
//  R77 (tau 3.47 s). Envelope accumulators widened to 24 bits after a
//  quantisation bug (see sv_changes.md 2026-08-12c).
//  PRIOR REVISION 2026-08-12b: FIVE NEW VOICES — LASER_1, LASER_2,
...
module astrob_audio (
    ...
    always @(posedge clk_sys or posedge reset) begin
        if (reset) begin
            ...
            vclk_cnt <= VSTEP_HALF;               // gate input LOW: NAND
                                                  // output pinned, osc held
    ...
    //------------------------------------------------------------------------
    // SONAR ($3F bit 7). Netlist lines 637-750.  ** MAME-CAPTURE CALIBRATED **
    //
    // Four op-amp state-variable oscillators (U1/U2 LM324 pairs, diode
    // limited -> near-sine, 2nd harmonic measured -39 dB) sum into the
    // Q2/Q3 differential VCA whose tail current (Q1, shaped by C47 4.7u /
    // R117 1k attack / R108 100k decay) is the ping envelope. Nominally all
    // four oscillators sit at the SAME frequency — the sound exists only
    // because real parts detune them, which is why this voice cannot be
    // derived from the schematic (MAME injects FRND randomised values).
    //
    // Constants below are calibrated against a MAME gameplay capture
    // (sonar.wav, 2026-08-12): lines 469.15 / 471.63 / 473.56 / 480.93 Hz
    // at relative levels -12.7 / -0.5 / 0 / -2.0 dB; attack 10-90% ~96 ms;
    // decay tau 478 ms (nominal R108*C47 = 470 ms — circuit agrees).
    // *** These encode MAME's random seed, not a real board. A hardware
    // capture supersedes this block wholesale; keep the envelope, replace
    // the four increments/weights. ***
    //------------------------------------------------------------------------
    reg        sonar_bit_d;
    always @(posedge clk_sys) sonar_bit_d <= latch_3f[7];
    wire sonar_trig = sonar_bit_d & ~latch_3f[7];

    // capture-fit pulse (attack completes; nominal one-shot R20*C4 = 34.7 ms
    // under-runs the measured envelope through the nonlinear Q1 turn-on)
    localparam [13:0] SONAR_TICKS = 14'd8459;      // 140 ms
    reg [13:0] sonar_pc;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) sonar_pc <= 14'd0;
        else if (sonar_trig) sonar_pc <= SONAR_TICKS;
        else if (tick && sonar_pc != 0) sonar_pc <= sonar_pc - 14'd1;
    end

    // 24-bit envelope accumulator: at 17-bit width the decay multiplier
    // (x9>>18) quantises to 0-2 LSB and the -1 floor dominates, shortening
    // the tail (measured tau 386 ms vs the 478 ms target). Seven fractional
    // bits restore the exponential. Value = top 17 bits.
    reg [23:0] sonar_env24;
    wire [16:0] sonar_env   = sonar_env24[23:7];
    wire [23:0] sonar_d_att = {ENV_MAX,7'd0} - sonar_env24;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) sonar_env24 <= 24'd0;
        else if (tick) begin
            if (sonar_pc != 0)                       // attack tau 68 ms (x2>>13, capture-fit)
                sonar_env24 <= sonar_env24 + (({2'b0,sonar_d_att}*2) >> 13);
            else if (sonar_env24 != 0)               // decay tau 482 ms (x9>>18)
                sonar_env24 <= sonar_env24 - (({2'b0,sonar_env24}*9) >> 18) - 24'd1;
        end
    end

    // four phase accumulators, 24-bit @ 60.42 kHz tick
    localparam [23:0] SON_INC0 = 24'd130264;   // 469.15 Hz
    localparam [23:0] SON_INC1 = 24'd130952;   // 471.63 Hz
    localparam [23:0] SON_INC2 = 24'd131488;   // 473.56 Hz
    localparam [23:0] SON_INC3 = 24'd133535;   // 480.93 Hz
    reg [23:0] son_ph0, son_ph1, son_ph2, son_ph3;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) begin
            // staggered reset phases: the board's oscillators free-run, so
            // by any real trigger their phases are uncorrelated. Coherent
            // (all-zero) starts make the first beat crest fully constructive
            // and audibly sharpen early pings.
            son_ph0 <= 24'h000000; son_ph1 <= 24'h5A0000;
            son_ph2 <= 24'h9C0000; son_ph3 <= 24'h2E0000;
        end else if (tick) begin
            son_ph0 <= son_ph0 + SON_INC0;
            son_ph1 <= son_ph1 + SON_INC1;
            son_ph2 <= son_ph2 + SON_INC2;
            son_ph3 <= son_ph3 + SON_INC3;
        end
    end

    // quarter-wave sine LUT, 64 x 12-bit
    localparam [767:0] SINLUT = {12'd2012, 12'd2021, 12'd2028, 12'd2035, 12'd2039, 12'd2043, 12'd2046, 12'd2047,
              12'd1901, 12'd1919, 12'd1936, 12'd1951, 12'd1966, 12'd1979, 12'd1992, 12'd2003,
              12'd1716, 12'd1743, 12'd1769, 12'd1793, 12'd1817, 12'd1840, 12'd1861, 12'd1881,
              12'd1465, 12'd1500, 12'd1533, 12'd1566, 12'd1598, 12'd1629, 12'd1659, 12'd1688,
              12'd1158, 12'd1199, 12'd1239, 12'd1279, 12'd1318, 12'd1356, 12'd1393, 12'd1430,
              12'd807, 12'd852, 12'd898, 12'd943, 12'd987, 12'd1031, 12'd1074, 12'd1116,
              12'd424, 12'd473, 12'd522, 12'd570, 12'd618, 12'd666, 12'd713, 12'd760,
              12'd25, 12'd75, 12'd126, 12'd176, 12'd226, 12'd275, 12'd325, 12'd375};
    function signed [12:0] sine_q;
        input [23:0] ph;
        reg   [5:0]  idx;
        reg   [11:0] mag;
        begin
            idx = ph[22] ? ~ph[21:16] : ph[21:16];   // fold quarter
            mag = SINLUT[idx*12 +: 12];
            sine_q = ph[23] ? -$signed({1'b0,mag}) : $signed({1'b0,mag});
        end
    endfunction

    // capture-derived amplitude weights, sum 6000
    wire signed [12:0] son_s0 = sine_q(son_ph0);
    wire signed [12:0] son_s1 = sine_q(son_ph1);
    wire signed [12:0] son_s2 = sine_q(son_ph2);
    wire signed [12:0] son_s3 = sine_q(son_ph3);
    wire signed [26:0] son_mix27 = son_s0*16'sd468 + son_s1*16'sd1907
                                 + son_s2*16'sd2020 + son_s3*16'sd1605;
    wire signed [15:0] son_mix = son_mix27 >>> 11;          // +/-6000
    wire signed [33:0] son_envP = son_mix * $signed({1'b0, sonar_env});
    // [33:16] slice: with ENV_MAX 61440 and son_mix +/-6000 the peak is
    // ~5625, so bits [33:32] are pure sign extension. If ENV_MAX or the
    // sonar weights ever grow, widen this slice or add saturation.
    wire signed [15:0] sonar_out = son_envP[33:16];         // env Q16

    //------------------------------------------------------------------------
    // BONUS ($3F bit 6) — the "deedle". Netlist lines 1134-1168, nominal.
    //
    // Trigger one-shot (R44 1M x C19 .05u = 34.7 ms) dumps C34 (10u, hung
    // from +12) through D7/R78 1k; C34 then recovers through R77 330k with
    // tau = 3.3 s — that recovery IS the envelope, self-gating the output
    // via the D8 half-wave stage (at rest the swing collapses to zero).
    // Tone: U6 555 (R74 4.7k / R75 100k / C31 4.7n) ~1500 Hz, with OSC4
    // (7.19 Hz, MAME hard clock) injecting into the pin-7 node through
    // R79 150k -> pitch alternates 1501 / 1448 Hz at 7.19 Hz. Peak swing
    // ~10 V of the 12 V rail -> amplitude scaled to ~5/6 VOICE_FS.
    // Netlist-nominal (no capture) — flag for ear check.
    //------------------------------------------------------------------------
    reg        bonus_bit_d;
    always @(posedge clk_sys) bonus_bit_d <= latch_3f[6];
    wire bonus_trig = bonus_bit_d & ~latch_3f[6];

    localparam [11:0] BONUS_TICKS = 12'd2097;      // 34.7 ms one-shot
    reg [11:0] bonus_pc;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) bonus_pc <= 12'd0;
        else if (bonus_trig) bonus_pc <= BONUS_TICKS;
        else if (tick && bonus_pc != 0) bonus_pc <= bonus_pc - 12'd1;
    end

    // 24-bit accumulator: at 17-bit width the x5>>20 decay term is
    // IDENTICALLY ZERO (61440*5 < 2^20) and the envelope decayed linearly
    // in ~1 s instead of exponentially in 3.5 s. Value = top 17 bits.
    reg [23:0] bonus_env24;
    wire [16:0] bonus_env   = bonus_env24[23:7];
    wire [23:0] bonus_d_att = {ENV_MAX,7'd0} - bonus_env24;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) bonus_env24 <= 24'd0;
        else if (tick) begin
            if (bonus_pc != 0)                       // C34 dump, tau ~10 ms
                bonus_env24 <= bonus_env24 + (bonus_d_att >> 9);
            else if (bonus_env24 != 0)               // R77*C34 tau 3.47 s (x5>>20)
                bonus_env24 <= bonus_env24 - (({3'b0,bonus_env24}*5) >> 20) - 24'd1;
        end
    end

    // 7.19 Hz chop selecting between the two 555 half-periods
    localparam [20:0] BON_CHOP_HALF = 21'd1075694;
    reg [20:0] bon_cc; reg bon_sel;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) begin bon_cc <= BON_CHOP_HALF; bon_sel <= 1'b0; end
        else if (bon_cc == 21'd0) begin bon_cc <= BON_CHOP_HALF; bon_sel <= ~bon_sel; end
        else bon_cc <= bon_cc - 21'd1;
    end

    localparam [12:0] BON_HALF_HI = 13'd5154;   // 1501 Hz (OSC4 high)
    localparam [12:0] BON_HALF_LO = 13'd5340;   // 1448 Hz (OSC4 low)
    reg [12:0] bon_tc; reg bon_sq;
    always @(posedge clk_sys or posedge reset) begin
        if (reset) begin bon_tc <= 13'd0; bon_sq <= 1'b0; end
        else if (bon_tc == 13'd0) begin
            bon_tc <= bon_sel ? BON_HALF_HI : BON_HALF_LO;
            bon_sq <= ~bon_sq;
        end else bon_tc <= bon_tc - 13'd1;
    end

    wire signed [24:0] bon_amp25 = $signed({1'b0,bonus_env}) * $signed(8'sd80);
    wire signed [15:0] bon_amp   = bon_amp25[24:10];   // 61440*80>>10 = 4800 pk
    wire signed [15:0] bonus_out = bon_sq ? bon_amp : -bon_amp;
